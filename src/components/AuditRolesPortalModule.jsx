...
export default function AuditRolesPortalModule({ patients, transactions, currentUser }) {
  ...
  const handleSearchDoctorPatient = (e) => {
    ...
    const match = patients.find(p => p.documentId.toLowerCase().includes(cleanId) || p.name.toLowerCase().includes(cleanId));
    if (match) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'doctor-portal' ? 'bg-teal-600 text-white shadow-sm' : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200'
          }`}
        >
            ...
            {foundPatient && (
              <div className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 text-xs">
                <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                  <div>
                    <h4 className="text-base font-extrabold text-slate-900">{foundPatient.name}</h4>
                    <p className="text-slate-600 font-mono">Cédula: {foundPatient.documentId} | Edad: {foundPatient.age} Años</p>
                  </div>
                  <span className="px-3 py-1 rounded-full bg-teal-100 text-teal-900 font-extrabold text-xs">
                    {foundPatient.category}
                  </span>
                </div>
                  ...
                  <div className="space-y-1.5">
                    {foundPatient.history.map((h, i) => (
                      <div key={i} className="p-2.5 bg-white border border-slate-200 rounded-xl flex justify-between items-center">
                        <div>
                          <div className="font-bold text-slate-900">{h.procedure}</div>
                          <div className="text-[10px] text-slate-500 font-mono">{h.date} - Tratante: {h.doctor}</div>
                        </div>
                        <span className="font-mono font-extrabold text-emerald-900 text-sm">${h.cost.toFixed(2)} USD</span>
                      </div>
                    ))}
                  </div>
                </div>
