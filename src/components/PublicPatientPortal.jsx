...
import { Globe, Link as LinkIcon, Check, Copy, Eye, EyeOff, Calendar, ShieldCheck, Clock, Phone, ExternalLink, Search, UserCheck, UserPlus, ShieldAlert, Layers, ToggleLeft, ToggleRight } from 'lucide-react';
import { CLINIC_INFO } from '../mockData';
import { createAppointmentApi, createPatientApi, createOrUpdateProcedureApi } from '../api';

export default function PublicPatientPortal({ procedures = [], setProcedures, specialists = [], patients = [], onAddAppointment, isPublicMode = false }) {
  ...
  const [showPerServiceSettings, setShowPerServiceSettings] = useState(false);
  ...
  // Conmutador de Visibilidad Individual por Servicio
  const handleToggleProcedurePublicVisibility = async (procObj) => {
    const updatedStatus = !(procObj.isPublicVisible !== false);
    const updatedProc = { ...procObj, isPublicVisible: updatedStatus };

    if (typeof setProcedures === 'function') {
      setProcedures(procedures.map(p => String(p.id) === String(procObj.id) ? updatedProc : p));
    }

    try {
      await createOrUpdateProcedureApi(updatedProc);
    } catch (e) {}
  };

  // Procedimientos visibles públicamente en Paso 1
  const filteredProcedures = useMemo(() => {
    const query = procedureSearch.trim().toLowerCase();
    // Filtro 1: Solo mostrar procedimientos cuya visibilidad individual sea pública (isPublicVisible !== false)
    const publicOnly = (procedures || []).filter(p => p.isPublicVisible !== false);

    if (!query) return publicOnly;
        ...
        <div className="bg-white dark:bg-[#111c3a] border border-slate-200 dark:border-[#1e2d5a] shadow-sm p-6 rounded-2xl space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-[#1e2d5a] pb-4">
            ...
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-[#0d162f] border border-slate-300 dark:border-[#1e2d5a] p-2 rounded-xl">
              ...
              <button
                onClick={handleCopyUrl}
                className="px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white font-extrabold rounded-lg text-xs flex items-center gap-1.5 shadow-sm transition-all shrink-0 active:scale-95 cursor-pointer"
              >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setShowPerServiceSettings(!showPerServiceSettings)}
                className="text-xs font-black text-teal-600 dark:text-teal-400 hover:underline flex items-center gap-1 cursor-pointer"
              >
                <Layers className="w-3.5 h-3.5" />
                {showPerServiceSettings ? 'Ocultar Selector de Servicios' : '⚙️ Configurar Visibilidad Servicio por Servicio'}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-slate-50 dark:bg-[#0d162f] border border-slate-200 dark:border-[#1e2d5a] rounded-xl flex items-center justify-between">
                <div>
                  <div className="font-extrabold text-xs text-slate-900 dark:text-white">Visibilidad General del Catálogo</div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Muestra u oculta el módulo de tratamientos completo.</div>
                </div>
                <button
              ...
              <div className="p-4 bg-slate-50 dark:bg-[#0d162f] border border-slate-200 dark:border-[#1e2d5a] rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Muestra u oculta las tarifas en USD al paciente.</div>
                </div>
                <button
            ...
            {/* SECCIÓN DESPLEGABLE DE SELECCIÓN DE VISIBILIDAD SERVICIO POR SERVICIO */}
            {showPerServiceSettings && (
              <div className="p-4 bg-slate-100 dark:bg-[#091024] border border-slate-300 dark:border-[#1e2d5a] rounded-2xl space-y-3 mt-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-black text-xs text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                      <Layers className="w-4 h-4 text-teal-600" />
                      Visibilidad Servicio por Servicio en Portal Público
                    </h4>
                    <p className="text-[11px] text-slate-500 font-medium">
                      Marca cuáles servicios de tu baremo deseas que aparezcan en el catálogo público y cuáles deseas mantener exclusivamente para uso interno de la clínica.
                    </p>
                  </div>
                  <span className="text-[11px] font-bold text-teal-700 bg-teal-100 dark:bg-teal-900/50 px-2 py-0.5 rounded-full">
                    {procedures.filter(p => p.isPublicVisible !== false).length} de {procedures.length} públicos
                  </span>
                </div>

                <div className="max-h-64 overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
                  {procedures.map(proc => {
                    const isVisible = proc.isPublicVisible !== false;
                    return (
                      <div
                        key={proc.id}
                        className="p-2.5 bg-white dark:bg-[#111c3a] border border-slate-200 dark:border-[#1e2d5a] rounded-xl flex items-center justify-between text-xs font-bold"
                      >
                        <div>
                          <div className="text-slate-900 dark:text-white flex items-center gap-2">
                            <span>{proc.name}</span>
                            <span className="text-[10px] text-slate-400 font-mono">({proc.code || proc.id})</span>
                          </div>
                          <div className="text-[10px] text-slate-500">
                            {proc.category || 'General'} | ${proc.price} USD
                          </div>
                        </div>

                        <button
                          type="button"
                          onClick={() => handleToggleProcedurePublicVisibility(proc)}
        ...
        {submittedSuccess ? (
          <div className="p-8 bg-emerald-50 dark:bg-teal-900/30 border border-emerald-300 dark:border-teal-700 rounded-2xl text-center space-y-3">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-extrabold rounded-xl text-xs mt-2 transition-all shadow-md cursor-pointer"
            >
              Agendar Otra Cita
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-6 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] font-bold text-slate-500">{(filteredProcedures || []).length} servicio(s) disponibles</span>
              </div>
                ...
                <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar">
                  {filteredProcedures.length === 0 ? (
                    <div className="p-4 bg-slate-50 dark:bg-[#0d162f] rounded-xl text-xs text-slate-500 text-center font-bold">
                      No se encontraron tratamientos públicos disponibles.
                    </div>
                  ) : (
            ...
            <form onSubmit={handlePublicAppointmentSubmit} className="lg:col-span-6 space-y-4 bg-slate-50 dark:bg-[#0d162f] p-5 rounded-2xl border border-slate-200 dark:border-[#1e2d5a] text-xs font-bold">
                ...
                <div className="space-y-2 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-300 dark:border-slate-700">
                  ...
                  <div className="pt-1 text-right">
                    <button
                      ...
                      className="text-[11px] text-teal-600 dark:text-teal-400 underline font-bold hover:text-teal-800 cursor-pointer"
                    >
              ...
              <button
                type="submit"
                className="w-full py-3 bg-teal-600 hover:bg-teal-700 text-white font-black rounded-xl shadow-md text-sm mt-3 transition-all flex items-center justify-center gap-2 active:scale-95 cursor-pointer"
              >
